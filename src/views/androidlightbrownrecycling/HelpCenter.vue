    ...
    export default {
		data() {
			return {
                ...
                listAnswer: [
                    {value:"莫愁周转是专注为中国大陆地区互联网用户提供的智能终端的回收服务平台。"},
                    ...
                    {value:"脏机（偷捡抢）、黑机（苹果套保机），不回收；不在莫愁周转回收类别中的设备，不回收。"},
                    {value:"莫愁周转为线上服务平台，不接受用户自行上门送货。"},
                    {value:"莫愁周转已接入数帮保险，机器确为运输途中损坏的，数帮保险为您承保订单金额50%的货款。"},
                    {value:"莫愁周转仅限回收当前登录设备，如您有多部设备，需在待回收的设备上安装app进行操作。"},
                    {value:"快递费用由用户自行承担，莫愁周转不接收到付的快递订单。"},
                    {value:"莫愁周转会在用户确认接受评估价格后24小时内，将订单金额的30%作为首付款，汇至用户已绑定的银行卡上; 在收到用户设备并检测后，将尾款汇至用户的银行卡上。"},
                    {value:"莫愁周转收到用户的机器后会进行检测, 机器需要与提交订单时硬件检测的信息一致，对于不一致的机器，平台拒绝回收。"},
                    {value:"对于未能通过检测的机器，莫愁周转会将机器快递给用户，运费需要用户自行承担。"},
                    {value:"需要退回的机器，莫愁周转平台使用顺丰快递快递给您，通常在3-5个工作日可以到达。"},
                ],
                questionNum: '',
                show: true
            }
